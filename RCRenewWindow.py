...
def viewDet(root, val):
    ...
    vr.main(val, loginusername)


def backfun(root):
    root.destroy()
    ren.main(loginusername)
...
# -----------Top Part----------------
    ...
    topLabel = Label(TopFrame, text="Request Pending for Renewal of Registration Certificate", padx=20, pady=40, font=("seogeui 25 bold"), bg="blue",
                     fg="white")
...
# -------------------------------------------------
    cur = openConnection()
    cur.execute("select rc.Aadhaar_number, Name, Mobile_num from rc, renewstatus where rc.Aadhaar_number=renewstatus.Aadhaar_number and renewstatus.rc_ren_status='PENDING'")
    lst = cur.fetchall()
    print(lst)
...
# ------------Creating a Parent Frame for table to reside -------------
    ...
    back = Button(MainBoard, text="Back", bg="white", fg="red", font=("consolas 20 bold"), activebackground="red", activeforeground="white", borderwidth=2, relief=SOLID, takefocus="red", command=lambda:backfun(root))
