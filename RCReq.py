...
# -----------Top Part----------------
    ...
    topLabel = Label(TopFrame, text="Request Pending for Approval of Registration Certificate", padx=20, pady=40, font=("seogeui 25 bold"), bg="blue",
                     fg="white")
...
# -------------------------------------------------
    cur = openConnection()
    cur.execute("select Aadhaar_number, Name, Mobile_num from rc, status where rc.Aadhaar_number=status.Aadhar_num and status.rc_status='PENDING'")
    lst = cur.fetchall()
    print(lst)
