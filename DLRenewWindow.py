...
def viewDet(root, val):
    ...
    ViewDLRenew.main(val, loginusername)
...
# -----------Top Part----------------
    ...
    topLabel = Label(TopFrame, text="Request Pending for Renewal of Driving License", padx=20, pady=40, font=("seogeui 25 bold"), bg="blue",
                     fg="white")
...
# -------------------------------------------------
    cur = openConnection()
    cur.execute("select Aadhar_number, Name, Mobile_num from license, renewstatus where license.Aadhar_number=renewstatus.Aadhaar_number and renewstatus.dl_ren_status='PENDING'")
    lst = cur.fetchall()
