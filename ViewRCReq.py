...
def generateRC(root, lst, valuelist, generate):
    ...
    exdate = datetime.datetime.today().strftime(f"{int(datetime.datetime.today().strftime('%Y')) + 5}-%m-%d")
...
def approveRC(root, adno):
    ...
    cur.execute(f"update status set rc_status = 'APPROVE' where aadhar_num='{adno}'")
