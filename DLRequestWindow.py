...
def viewDet(root, val):
    ...
    Viewdlreq.main(val, loginusername)
...
# -----------Top Part----------------
    ...
    topLabel = Label(TopFrame, text="Request Pending for Approval of Driving License", padx=20, pady=40, font=("seogeui 25 bold"), bg="blue",
                     fg="white")
...
# -------------------------------------------------
    cur = openConnection()
    cur.execute("select Aadhar_number, Name, Mobile_num from license, status where license.Aadhar_number=status.Aadhar_num and status.dl_status='PENDING'")
    lst = cur.fetchall()
