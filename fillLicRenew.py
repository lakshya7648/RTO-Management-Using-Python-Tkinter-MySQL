...
def viewDetails(aadhaar, mobno, fname):
    ...
    sqlst = getCon()
    cur=sqlst[0]
    s1 = cur.execute(f"select Aadhar_number, name, Mobile_num, DL_No from license where Aadhar_number = '{aadhaar.get()}'")
    lst = cur.fetchone()
    print(lst)
    if s1 == 1:
        if lst[3] == None:
...
def submitDet(aadhaar, mobno, fname):
    ...
    s = cur.execute(f"insert into renewstatus values('{aadhaar.get()}', 'PENDING', Null)")
    con.commit()
    s=cur.execute(f"update license set mobile_num = '{mobno.get()}' where aadhar_number='{aadhaar.get()}'")
    print(s)
    con.commit()
    s=cur.execute(f"update license set name = '{fname.get()}' where aadhar_number='{aadhaar.get()}'")
    con.commit()
...
def main():
    root = Tk()
    root.title("License Renew Window")
    root.state("zoomed")
    ...
    topLabel = Label(TopFrame, text="License Renew Window", padx=20, pady=40, font=("seogeui 25 bold"), bg="green",
                     fg="white")
